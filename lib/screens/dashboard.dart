         ...
         return Sizer(
           builder: (context,orientation,deviceType){
            if (snapshot.connectionState == ConnectionState.done) {
               // If we got an error
                if (snapshot.hasError || (snapshot.hasData && snapshot.data.toString()=="0" )) {
                    return  Center(
                          ...
                          Expanded(
                            child: Padding(
                              ...
                              child: ListView.builder(
                                  itemCount: EventResponse.eventResponse.eventItem.length,
                                  itemBuilder: (BuildContext context,int index){
                                    return Padding(
                                      padding: const EdgeInsets.fromLTRB(25, 3, 25, 3),
                                      child: InkWell(
                                        onTap: (){
                                          Navigator.pushNamed(context, '/eventpage',arguments: {'index':index});
                                        },
                                        child: Container(
                                          ...
                                          child: Center(
                                            child: Text(
                                              "${EventResponse.eventResponse.eventItem[index].title}",
                                              style: TextStyle(
                                                  letterSpacing: 0.8,
                                                  fontSize: 13.sp,
                                                  fontWeight: FontWeight.w500
